    ...
    test('calls saveConfig and updates localStorage for valid keys', async ({ page }) => {
        const result = await page.evaluate(async () => {
            ...
            localStorage.setItem = (key, value) => {
                ...
                savedKey = key;
                savedValue = value;
                // Don't actually call original to avoid polluting state,
                // but if we wanted to we would do originalSetItem(key, value);
            };

            // Setup a non-default value to ensure saveConfig detects a change (if it diffs)
            // or just rely on resetToDefault calling saveConfig directly.
