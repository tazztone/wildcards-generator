...
import { sanitize, escapeHtml, debounce, truncate } from './utils.js';
...
export const UI = {
    ...
    getCategoryFolderHtml(name, data, path) {
        ...
        return `
            <summary class="flex justify-between items-center p-2 cursor-pointer gap-2 group/catheader" title="${escapeHtml(tooltipText)}">
                <div class="flex items-center gap-2 flex-grow min-w-0">
                    ...
                    <h2 class="text-lg font-semibold text-accent select-none editable-wrapper flex-shrink-0"><span class="editable-name category-name outline-none rounded px-0.5" tabindex="0">${sanitize(name.replace(/_/g, ' '))}</span><span class="edit-icon">✏️</span></h2>${templateBadge}
                    <input type="text" 
    ...
    getWildcardCardHtml(name, data, path) {
        ...
        return `
            ...
            <div class="flex items-center gap-2 mb-1 group/header" title="${escapeHtml(tooltipText)}">
                ...
                <h3 class="font-bold text-sm text-gray-100 editable-wrapper flex-shrink-0"><span class="editable-name wildcard-name outline-none rounded px-0.5" tabindex="0">${sanitize(name.replace(/_/g, ' '))}</span><span class="edit-icon">✏️</span></h3>
                <input type="text" 
    ...
    createChip(wildcard, index, path = null) {
        ...
        return `<div class="${classes}" data-index="${index}" tabindex="0" role="checkbox" aria-checked="false" aria-label="Select ${escapeHtml(wildcard)}"><span class="editable-name chip-text outline-none rounded px-0.5" aria-label="Double-click to edit item">${sanitize(wildcard)}</span></div>`;
    },
...
<div class="text-left space-y-4 max-w-lg custom-scrollbar max-h-[70vh] overflow-y-auto pr-2">
    ...
    <section class="border-t border-indigo-500/20 pt-3">
        ...
        <div class="grid grid-cols-2 gap-2 text-sm bg-gray-900/40 rounded-lg p-3 border border-gray-800">
            ...
            <div><kbd class="px-2 py-1 bg-gray-700 rounded text-xs">?</kbd> <span class="text-gray-500 ml-1">Help</span></div>
        </div>
    </section>
