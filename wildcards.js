...
// =================================================================================
const State = {
    appState: { systemPrompt: '', suggestItemPrompt: '', wildcards: {}, pinnedCategories: [] },
    history: [],
    historyIndex: -1,
    ...
    async saveConfig() {
        try {
            ...
            UI.showToast('Configuration saved. Some changes may require a reload.', 'success');
        } catch (error) {
    ...
    resetState(notify = true) {
        const performReset = (data) => {
            ...
            UI.renderAll();
            if (notify) UI.showToast('All categories have been reset.', 'info');
        };
...
const Api = {
    ...
    async testConnection(provider) {
        UI.showToast(`Testing connection to ${provider}...`, 'info');
        try {
...
const UI = {
    ...
    renderAll() {
        ...
        const pinnedCategories = State.appState.pinnedCategories || [];
        const sortedKeys = Object.keys(wildcards).sort((a, b) => {
    ...
    updateStats() {
        ...
        countData(State.appState.wildcards);

        const pinnedCount = (State.appState.pinnedCategories || []).length;
        ...
        document.getElementById('stat-pinned').textContent = pinnedCount;
    },
    createCategoryElement(name, data, level, path, index) {
        ...
        // Lazy loading: render placeholder first, actual content on expand
        const isTopLevel = level === 0;
        if (isTopLevel) {
            // Add loading placeholder
            contentWrapper.innerHTML = '<div class="lazy-placeholder text-gray-500 text-center py-4">Loading...</div>';
            element.dataset.rendered = 'false';

            // Render content on first toggle
            element.addEventListener('toggle', () => {
                if (element.open && element.dataset.rendered === 'false') {
                    this.renderCategoryContent(element, data, path, level);
                    element.dataset.rendered = 'true';
                }
            }, { once: true });
        } else {
            // Non-top-level: render immediately for nested navigation
            this.renderCategoryContent(element, data, path, level);
            element.dataset.rendered = 'true';
        }
    ...
    getCategoryFolderHtml(name, data, path) {
        const isPinned = State.appState.pinnedCategories && State.appState.pinnedCategories.includes(path);
        return `
                    <summary class="flex justify-between items-center p-4 cursor-pointer gap-4">
                        <div class="flex items-center gap-3 flex-wrap flex-grow">
                            ...
                            <button class="pin-btn text-yellow-400 hover:text-yellow-300 text-lg" title="${isPinned ? 'Unpin' : 'Pin to top'}">${isPinned ? '📌' : '📍'}</button>
...
const App = {
    ...
    initTheme() {
        const savedTheme = localStorage.getItem('wildcards-theme') || 'dark';
    ...
    bindEventListeners() {
        ...
        document.addEventListener('click', e => {
            ...
            if (e.target.matches('#check-duplicates')) this.handleCheckDuplicates();
        ...
        // Update batch UI when individual checkboxes change
    ...
    handleKeyboardShortcuts(e) {
        ...
        if (['ArrowUp', 'ArrowDown', 'Enter', 'Escape'].includes(e.key)) {
            ...
            if (e.key === 'ArrowDown') {
                ...
                currentCategory.dispatchEvent(new Event('toggle'));
    ...
    handleTogglePin(path) {
        if (!State.appState.pinnedCategories) {
            State.appState.pinnedCategories = [];
        }
        const index = State.appState.pinnedCategories.indexOf(path);
        if (index === -1) {
            State.appState.pinnedCategories.push(path);
            UI.showToast(`Pinned "${path.replace(/_/g, ' ')}" to top`, 'success');
        } else {
            State.appState.pinnedCategories.splice(index, 1);
            UI.showToast(`Unpinned "${path.replace(/_/g, ' ')}"`, 'info');
        }
        State.saveState();
        UI.renderAll();
    },
    handleCopy(path) { const content = this.getObjectByPath(path).wildcards.join('\n'); navigator.clipboard.writeText(content).then(() => UI.showToast('Copied to clipboard!', 'success')).catch(() => UI.showToast('Failed to copy.', 'error')); },
            ...
            UI.showNotification(`Delete ${categories.length} selected categories?`, true, () => {
                ...
                categories.forEach(cat => {
                    if (cat && cat.dataset.path) {
                        ...
                        const parent = parts.length > 0 ? this.getObjectByPath(parts.join('/')) : State.appState.wildcards;
                ...
                if (el.dataset.path) {
                    el.querySelectorAll('.chip span').forEach(span => {
                        ...
                        if (originalText.toLowerCase().includes(term)) {
                            ...
                            span.innerHTML = sanitize(originalText).replace(regex, `<mark class="search-highlight">$1</mark>`);
                        }
                    });
                }
            } else {
    ...
    handleDownloadZip() {
        ...
        const addFilesToZip = (data, path) => {
            Object.keys(data).filter(k => k !== 'instruction').forEach(key => {
                ...
                if (isLeaf) {
                    // Create file in proper folder path
                    zip.file(`${currentPath}.txt`, currentData.wildcards.join('\n'));
                } else {
        ...
        zip.generateAsync({ type: "blob" }).then(content => {
            ...
            UI.showToast('ZIP downloaded with organized folders!', 'success');
        }).catch(err => {
            console.error('ZIP generation error:', err);
            UI.showToast('Failed to generate ZIP file.', 'error');
        });
    },
    ...
    handleExportYaml() {
        try {
            ...
            a.click();
            URL.revokeObjectURL(a.href);
            UI.showToast('Wildcards exported as YAML.', 'success');
        } catch (error) {
            ...
            reader.onload = re => {
                ...
                try {
                    ...
                    UI.showNotification('Importing this file will overwrite your current collection. Are you sure?', true, () => {
                        ...
                        State.saveStateToHistory();
                        UI.renderAll();
                        UI.showToast('YAML file imported successfully.', 'success');
                    });
    ...
    handleCheckDuplicates() {
        const wildcardMap = new Map(); // wildcard -> [paths where it appears]
            ...
            Object.keys(data).filter(k => k !== 'instruction').forEach(key => {
                ...
                if (item.wildcards && Array.isArray(item.wildcards)) {
                    item.wildcards.forEach(wildcard => {
                        ...
                        if (!wildcardMap.has(normalized)) {
                            wildcardMap.set(normalized, []);
                        }
        ...
        scanData(State.appState.wildcards, '');

        // Find duplicates (entries with more than one location)
        const duplicates = [];
        wildcardMap.forEach((locations, wildcard) => {
            if (locations.length > 1) {
                duplicates.push({ wildcard: locations[0].original, count: locations.length, locations });
            }
        });

        if (duplicates.length === 0) {
            UI.showToast('No duplicates found across all wildcards!', 'success');
        } else {
            const summary = duplicates.slice(0, 10).map(d =>
                `<b>${sanitize(d.wildcard)}</b> (${d.count}x): ${d.locations.slice(0, 3).map(l => l.path.split('/').pop()).join(', ')}${d.count > 3 ? '...' : ''}`
            ).join('<br>');

            const message = `Found <b>${duplicates.length}</b> duplicate wildcard${duplicates.length > 1 ? 's' : ''}:<br><br>${summary}${duplicates.length > 10 ? '<br><br>..and more' : ''}`;
            UI.showNotification(message);
        }
    },
